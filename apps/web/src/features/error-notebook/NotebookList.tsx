...
const CATEGORIES: NotebookCategory[] = ['Grammar', 'Vocabulary', 'Pronunciation', 'Business Context'];

export const NotebookList: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<NotebookCategory | 'All'>('All');

  const { data: entries, isLoading } = useQuery({
    queryKey: ['notebook', selectedCategory],
    queryFn: () => getNotebookEntries(selectedCategory === 'All' ? undefined : selectedCategory),
  });

  if (isLoading) return <div className={styles.loading}>Loading your notebook...</div>;
          ...
          entries?.map(entry => (
            <div key={entry.id} className={styles.entryCard}>
              <div className={styles.entryHeader}>
                ...
                <span className={styles.recurrence}>Seen {entry.recurrenceCount} times</span>
              </div>
