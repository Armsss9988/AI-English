...
public class ContentItem
{
    ...
    public static ContentItem CreateScenario(string title, string description, string goal, string persona)
    {
        ...
        item._versions.Add(ContentVersion.CreateScenarioInitial(item.Id, 1, title, description, goal, persona));
        return item;
    }
    ...
    public ContentVersion CreateNewVersion(string content, string? category = null, string? usageExample = null)
    {
        ...
        CurrentVersion++;
        var version = ContentVersion.Create(Id, CurrentVersion, Title, content, category, usageExample);
