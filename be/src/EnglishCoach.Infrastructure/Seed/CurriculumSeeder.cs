...
public class CurriculumSeeder
{
    ...
    private readonly Dictionary<Guid, ContentItem> _itemsById = new();
    ...
    public IReadOnlyDictionary<Guid, ContentItem> ItemsById => _itemsById;
    ...
    private void SeedScenarios()
    {
        ...
        {
            // Standup (5)
            ("Daily Standup - Regular", "standup", "Regular daily standup meeting", "Share daily updates", "Client PM who attends daily standups"),
            ("Async Standup Update", "standup", "Chat-based standup update", "Provide written updates", "Remote client in different timezone"),
            ("Blocked Status Report", "standup", "Reporting a blocker", "Communicate obstacles clearly", "Tech lead who removes blockers"),
            ("End-of-Sprint Summary", "standup", "Sprint wrap-up meeting", "Summarize sprint accomplishments", "Scrum master"),
            ("Absence Notice", "standup", "Vacation/absence notification", "Inform team of unavailability", "Team lead"),

            // Issue (5)
            ("Critical Bug Report", "issue", "Reporting a production bug", "Communicate severity and impact", "On-call engineer"),
            ("Production Incident", "issue", "Live production issue", "Escalate and coordinate response", "SRE/DevOps"),
            ("Third-Party Failure", "issue", "External dependency outage", "Report impact and workaround", "Integration specialist"),
            ("Performance Degradation", "issue", "System slowness reported", "Diagnose and communicate timeline", "Performance engineer"),
            ("Security Vulnerability", "issue", "Security issue discovered", "Report and coordinate fix", "Security team"),

            // Clarification (5)
            ("Requirement Ambiguity", "clarification", "Unclear requirements", "Get clarity on scope", "Business analyst"),
            ("Acceptance Criteria Gap", "clarification", "Missing acceptance criteria", "Define done criteria", "QA lead"),
            ("Conflicting Instructions", "clarification", "Received conflicting directions", "Resolve contradiction", "Project manager"),
            ("Scope Clarification", "clarification", "Feature scope unclear", "Define boundaries", "Product owner"),
            ("Technical Approach Question", "clarification", "Implementation approach needed", "Get technical guidance", "Solution architect"),

            // ETA (5)
            ("Initial Estimate Request", "eta", "Client asks for timeline", "Provide realistic estimate", "Project manager"),
            ("Delay Notification", "eta", "Informing about a delay", "Manage expectations", "Delivery lead"),
            ("Rush Request Response", "eta", "Responding to expedite request", "Negotiate scope or timeline", "Account manager"),
            ("Milestone Adjustment", "eta", "Requesting timeline change", "Justify and propose new date", "Program manager"),
            ("Delivery Confirmation", "eta", "Confirming delivery date", "Commit to timeline", "Technical lead"),

            // Summary (5)
            ("End-of-Day Summary", "summary", "Daily wrap-up", "Summarize day accomplishments", "Team lead"),
            ("Weekly Status Report", "summary", "Weekly progress report", "Communicate weekly progress", "Project manager"),
            ("Sprint Retrospective", "summary", "Sprint review meeting", "Present sprint results", "Scrum master"),
            ("Project Completion", "summary", "Project go-live summary", "Celebrate and document", "Delivery manager"),
            ("Handover Documentation", "summary", "Knowledge transfer", "Ensure smooth handover", "Technical writer"),
        };

        foreach (var (title, group, description, goal, persona) in scenarios)
        {
            var scenario = ContentItem.CreateScenario(title, description, goal, persona);
            scenario.SubmitForReview();
            scenario.Publish();
            _scenarios.Add(scenario);
            _itemsById[scenario.Id] = scenario;
        }
    }
    ...
    public Dictionary<string, int> PhrasesByCategory =>
        _phrases
    ...
    public Dictionary<string, int> ScenariosByGroup =>
        _scenarios
            .SelectMany(s => s.Versions)
            .Where(v => v.Goal != null)
            .GroupBy(v => GetScenarioGroup(v.Goal!))
            .ToDictionary(g => g.Key, g => g.Count());

    private static string GetScenarioGroup(string goal) =>
        goal.ToLower() switch
        {
            var g when g.Contains("standup") => "standup",
            var g when g.Contains("bug") || g.Contains("incident") || g.Contains("issue") => "issue",
            var g when g.Contains("clarif") => "clarification",
            var g when g.Contains("eta") || g.Contains("timeline") || g.Contains("delivery") => "eta",
            var g when g.Contains("summar") || g.Contains("report") || g.Contains("handover") => "summary",
            _ => "other"
        };
}